...
module.exports = exports = function(opts) {
  var force = opts.force
    , remove = opts.remove;
  Client.getJSON('storage/files', {}, function(err, res) {
    ...
    queries.push(function(cb) {
      async.each(files, function(file, cb) {
        var filePath = path.join(process.cwd(), file.filename);
        fs.stat(filePath, function(err, stat) {
          ...
          var request = Client.request('get', '/storage/file' + file.filename);
          request({ encoding: null }, function(err, res, body) {
            ...
            fs.outputFile(filePath, body, function(err) {
              if (err) return cb(err);
    ...
    async.parallel(queries, function(err) {
      ...
      console.log('\t%s files deleted', colors.blackBright(cRemoved));
