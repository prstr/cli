    ...
    async.parallel([
      ...
      if (missingFiles.length) {
        if (opts.remove) {
          ...
          missingFiles.forEach(function(file, i) {
            console.log('%s %s', colors.red(i + 1), file.path);
          });
          console.log('\nИспользуйте %s, чтобы удалить их с сервера\n', colors.yellow('--remove'));
        }
      }
      ...
      if (!opts.force && diff.updated.length) {
        console.log('\nСледующие файлы не будут отправлены, т.к. имеют более позднюю дату модификации на сервере:\n');
        diff.updated.forEach(function(file, i) {
          console.log('%s %s', colors.gray(i + 1), file.path);
        });
        console.log('\nИспользуйте %s, чтобы перезаписать их файлами из локальной версии, либо загрузите их с сервера с помощью %s.\n',
          colors.yellow('--force'),
      ...
      async.series(queries, function(err) {
        ...
        console.log('-------------');
        console.log('%s отправлено', colors.green(stats.pushed));
        console.log('%s удалено', colors.red(stats.removed));
        console.log('%s пропущено', colors.gray(stats.skipped));
        if (stats.failed)
          console.log('%s ошибок', colors.red(stats.failed));
        console.log('-------------\n');
        done();
      });
    });
